...
"""Isolates vm from all participants. Implemented as setting a tag in DO
with rules added to cloud. Indended to run by orgs"""
...
def main():
    ...
    if not do_api.remove_tag_by_vmname(token, get_image_name(TEAM, VMNUM), "team-image"):
        log_stderr("failed to remove tag team-image-closed from droplet")
        return 1

    if not do_api.add_tag_by_vmname(token, get_image_name(TEAM, VMNUM), "team-image-closed"):
