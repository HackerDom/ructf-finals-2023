...
function is_exists(conn::Redis.RedisConnection, key)
    return Redis_get(conn, key) != nothing
end
...
function create_user(conn::Redis.RedisConnection, username, password)
    ...
    Redis_set(conn, get_user_key(username), password_hash_64)
end
...
function is_valid_user_pair(conn::Redis.RedisConnection, username, password)
    expected_password_hash_64 = Redis_get(conn, get_user_key(username))
...
function create_field(conn::Redis.RedisConnection, field_record)
    ...
    Redis_set(conn, get_field_key(field_uuid), JSON.json(field_record))
    Redis_rpush(conn, joinpath("username2fields", field_record["owner"]), string(field_uuid))
